...
def save_images_2_array(pos_dir, neg_dir):
    ...
    for root, dirs, files in os.walk(pos_dir, topdown=False):
        ...
        for file in files:
            ...
            if not features_array_flag:         #第一帧图片输入时，新建features_array
                features_array = np.array(img_feature)
                features_array_flag = True
            else:
                features_array = np.row_stack((features_array, img_feature))     #之后就压入features_array中就行了
            cnt += 1
            print("pos " + str(cnt))
    ...
    for root, dirs, files in os.walk(neg_dir, topdown=False):
        ...
        for file in files:
            ...
            if not features_array_flag:
                neg_features_array = np.array(img_feature)
                features_array_flag = True
            else:
                neg_features_array = np.row_stack((neg_features_array, img_feature))
            cnt += 1
            print("neg " +str(cnt))
    ...
    np.save('neg_feature_data_array', neg_features_array)
...
if __name__ == '__main__':
    '''
    print('123')
    pos_dir = "H:\\dataset\\Tensor\\1529592066\\image\\"
    cnt = 0
    features_array_flag = False
    for root, dirs, files in os.walk(pos_dir, topdown=False):
        if pos_num == 0:
    ...
    #save_images_2_array("pos\\","neg\\")
    save_images_2_array("test\\", "test\\")
    c = np.load("feature_data_array.npy")
    print(c)
    #print(len(c))
    #print(len(c[0]))